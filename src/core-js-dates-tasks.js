...
function isDateInPeriod(date, period) {
  ...
  const startDateTime = Date.UTC(
    startDate.getUTCFullYear(),
    startDate.getUTCMonth(),
    startDate.getUTCDate()
  );

  const endDateTime = Date.UTC(
...
function getCountWeekendsInMonth(month, year) {
  ...
  for (let i = 1; i <= totalInMonth; i += 1) {
...
function getWeekNumberByDate(date) {
  ...
  const weekDay = dateCopy.getDay() || 7;

  dateCopy.setDate(dateCopy.getDate() - weekDay + 4);

  const januaryFirst = new Date(dateCopy.getFullYear(), 0, 1);

  const differenceInDays = Math.floor(
    (dateCopy - januaryFirst) / (24 * 60 * 60 * 1000)
  );
...
function getNextFridayThe13th(date) {
  ...
  resultingDate.setDate(13);
  if (resultingDate <= date) {
    resultingDate.setMonth(resultingDate.getMonth() + 1);
  }

  while (resultingDate.getDay() !== 5) {
    resultingDate.setMonth(resultingDate.getMonth() + 1);
  }
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  ...
  const parseDatePeriod = (value) => {
    const [periodDay, periodMonth, periodYear] = value.split('-').map(Number);
    return new Date(periodYear, periodMonth - 1, periodDay);
  };
  ...
  const start = parseDatePeriod(period.start);
  const end = parseDatePeriod(period.end);
  let current = new Date(start);

  while (current <= end) {
    for (let i = 0; i < countWorkDays && current <= end; i += 1) {
      result.push(formatGivenDate(current));
